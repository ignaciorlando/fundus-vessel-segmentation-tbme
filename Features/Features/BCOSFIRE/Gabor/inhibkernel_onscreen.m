function result = inhibkernel_onscreen(img, sigma, k1, k2, perc, invertOutput)
% VERSION 14/06/04
% CREATED BY: M.B. Wieling and N. Petkov, Groningen University,
%             Department of Computer Science, Intelligent Systems
%
% INHIBKERNEL_ONSCREEN: displays the inhibitionkernel on the screen in the
%   same size as the image. The inhibitionkernel is half-wave rectified before
%   display.
%   INHIBKERNEL_ONSCREEN(IMG, SIGMA, K1, K2, PERC) with the following parameters
...
% return a matrix initialized with the inhibitionkernel of size 2*N+1
inhibkernel = inhibkernel2D(sigma, k1, k2);

% apply half-wave rectification
inhibkernel = halfwaverect(perc,inhibkernel);
...
% Since the size in both directions of the inhibkernel is 2*N-1 the value of N can be easily
% calculated.
n = (size(inhibkernel,1) - 1)/2;

% create a 'h' x 'w' matrix with 0's 
matrixS = zeros(mxval);
matrixS = matrixS(1:h, 1:w);

% calculate where the inhibkernel in this matrix should be placed to be centered
% therefor we have to calculate the center of matrixS & subtract (begin) or add (end)
% the half of the size of the inhibkernel (=n). We have to add 1 because the index starts 
...
% place the inhibkernel in the center of matrixS, if the inhibkernel is larger than matrixS
% the inhibkernel is cropped in matrixS. The overlap (evenly on all sides)
% - calculated by the difference between the maxima (n and extremeh & extremew)
% is cut of so that the middle of the inhibkernel is still at (0,0).
overlaph = round(max(0,size(inhibkernel,1) - h)/2); % if there is overlap this value > 0
overlapw = round(max(0,size(inhibkernel,2) - w)/2); % if there is overlap this value > 0

if (overlaph <= 0) & (overlapw <= 0) % there is no overlap so the inhibkernel fits in matrixS
  matrixS(startvh:endvh, startvw:endvw) = inhibkernel;
elseif (overlaph <= 0) & (overlapw > 0) % there is overlap on the left and right side: crop
  ...
  matrixS(startvh:endvh, startvw:endvw) = inhibkernel(:, startindexw:endindexw);
elseif (overlaph > 0) & (overlapw <= 0) % there is overlap on the top and bottom side: crop
  ...
  matrixS(startvh:endvh, startvw:endvw) = inhibkernel(startindexh:endindexh, :);
else % inhibkernel > matrixS so we have to crop it
  startindexh = 1 + overlaph;
  endindexh = 2*extremeh + overlaph;
  startindexw = 1 + overlapw;
  endindexw = 2*extremew + overlapw;
  matrixS(startvh:endvh, startvw:endvw) = inhibkernel(startindexh:endindexh, startindexw:endindexw);
end
