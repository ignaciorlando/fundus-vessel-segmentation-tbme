    ...
    if config.learn.theta_p
        ...
        for i = 1 : config.features.numberFeatures
            ...
            for j = config.theta_p.initialValue : config.theta_p.increment : config.theta_p.lastValue
                ...
                if (getfield(qualityOverValidation, config.modelSelectionMetric) >= quality_best_theta_p_search)
                    quality_best_theta_p_search = getfield(qualityOverValidation, config.modelSelectionMetric);
                    theta_p_best = config.theta_p.values;
                end                    
                % Update the theta_p table
                config.theta_p.ThetaPValuesTable(i,countt) = getfield(qualityOverValidation, config.modelSelectionMetric);
